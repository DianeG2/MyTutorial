...
void modifyStaff(ManagementSystem* system) {
    ...
    Position position = ((Staff*)staff)->position;
    if (position != SALESMAN && position != SALESMANAGER) {
        printf("该员工不是销售人员，无法修改销售额。\n");
        return;
    }

    if (position == SALESMAN) {
        Salesman* salesman = (Salesman*)staff;
        printf("当前销售额：%.2f\n", salesman->sales);
        printf("请输入新的销售额：");
        scanf("%lf", &(salesman->sales));
    } else if (position == SALESMANAGER) {
        SalesManager* salesManager = (SalesManager*)staff;
        printf("当前销售额：%.2f\n", salesManager->sales);
        printf("请输入新的销售额：");
        scanf("%lf", &(salesManager->sales));
    }
...
void recombineFile(ManagementSystem* system) {
    FILE* outfile = fopen("StaffManagement.txt", "w");
    Node* current = system->head;
    while (current != NULL) {
        if (current->isEmployed) {
            void* staff = current->staff;
